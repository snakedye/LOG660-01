...
public class LectureBD {
    ...
    private void insertionPersonne(
        ...
        String insertSQL =
            "INSERT INTO Personnes (id_Personne, prenom, nom, email, telephone, " +
            "adresse_Num_Civique, adresse_Rue, adresse_Ville, adresse_Province, " +
            "adresse_Code_Postal, date_Naissance, mot_De_Passe) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(insertSQL)) {
            ...
            pstmt.setString(9, "");
            pstmt.setString(10, "");
            pstmt.setDate(11, new java.sql.Date(dateNaissance.getTime()));
            pstmt.setString(12, "");
    ...
    private void insertionFilm(
        ...
        String insertSQL =
            "INSERT INTO films (id_Film, titre, annee, resume, duree_minute, id_realisateur, id_langue) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(insertSQL)) {
            ...
            pstmt.setInt(6, realisateurId);
            // pstmt.setString(7, langue);

            int affectedRows = pstmt.executeUpdate();
            if (affectedRows > 0) {
                System.out.println(
                    "Film inséré avec succès : Titre = " + titre
                );
            }
    ...
    private void insertionClient(
        ...
        String insertSQL =
            "INSERT INTO Locations (id_Client, id_Film, date_Location, status) VALUES (?, ?, ?, ?)";

        try (PreparedStatement pstmt = connection.prepareStatement(insertSQL)) {
            pstmt.setInt(1, idClient);
            pstmt.setInt(2, idFilm);
            pstmt.setDate(3, new java.sql.Date(dateLocation.getTime()));
            pstmt.setString(4, "disponible"); // Assuming initial status is 'disponible'
